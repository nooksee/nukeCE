 ...
 ******************************************************/
    ...
    if(is_admin($admin)) {
        ...
        for ($i=0; $i < sizeof($clist); $i++) {
          if($clist[$i]!="") {
            if ($coverimage == $clist[$i]) {
              $sel = "selected";
            } else {
              $sel = "";
            }
            echo "<option name=\"cover\" value=\"$clist[$i]\" $sel>$clist[$i]";
          }
        }
        echo "</select><br>";
...
function preview_review($date, $title, $text, $reviewer, $email, $score, $cover, $url, $url_title, $hits, $id, $rlanguage) {
     ...
     ******************************************************/
        ...
        $day = substr($date,8,2);
	$fdate = NukeDate($board_config['default_dateformat'], mktime (0,0,0,$month,$day,$year), $board_config['board_timezone']);
...
/*****[BEGIN]******************************************
 [ Mod:     Reviews BBCodes                    v1.0.0 ]
 ******************************************************/
    $text = BBCode2Html(stripslashes($myrow['text']));
/*****[END]********************************************
 [ Mod:     Reviews BBCodes                    v1.0.0 ]
 ******************************************************/
    $cover = $myrow['cover'];
