...
//        $priority = 'group;user';
    ...
    */
    function handle_attachments($mode)
    {
        ...
        // nothing, if the user is not authorized or attachment disabled
                ...
                if ($this->filename != '')
                {
                    if ($this->num_attachments < intval($max_attachments))
                    {
                        ...
                        if (!$error && $this->post_attach)
                        {
                            ...
                            // This Variable is set to FALSE here, because Attachment enters Attachments into the
            ...
            else if ($edit_attachment || $update_attachment || $add_attachment || $preview)
            {
                ...
                if ($update_attachment)
                {
                    ...
                    if (!$error)
                    {
                        ...
                        $sql = 'UPDATE ' . ATTACHMENTS_DESC_TABLE . ' SET ' . attach_sql_build_array('UPDATE', $sql_ary) . '
                            WHERE attach_id = ' . (int) $attachment_id;
    ...
    */
        ...
        if ($mode == 'attach_list')
        {
			for ($i = 0; $i < sizeof($this->attachment_list); $i++)
            {
                if ($this->attachment_id_list[$i])
                {
                    ...
                    $sql = 'UPDATE ' . ATTACHMENTS_DESC_TABLE . "
                        SET comment = '" . attach_sql_escape($this->attachment_comment_list[$i]) . "'
                ...
                else
                {
                    ...
                    $sql = 'INSERT INTO ' . ATTACHMENTS_DESC_TABLE . ' ' . attach_sql_build_array('INSERT', $sql_ary);
                    ...
                    $sql = 'INSERT INTO ' . ATTACHMENTS_TABLE . ' ' . attach_sql_build_array('INSERT', $sql_ary);
    ...
    /**
    * Attachment entry switch/output (intern)
    * @private
    */
    function display_attachment_bodies()
    {
        ...
        $template->assign_vars(array(
            ...
            'L_RULES'                => $lang['Allowed_extensions_and_sizes'],
            'U_RULES' => append_sid("attach_rules.$phpEx?f=$u_rules_id&popup=1"),
            'S_HIDDEN' => $s_hidden)
        );
        ...
        if ($this->post_attach)
        {
            ...
            $sql = 'SELECT g.allow_group, g.max_filesize, g.cat_id, g.forum_permissions
                ...
                WHERE g.group_id = e.group_id
                    AND e.extension = '" . attach_sql_escape($this->extension) . "'
                LIMIT 1";
...
function execute_posting_attachment_handling()
{
    ...
    $attachment['posting'] = new attach_posting();
    $attachment['posting']->posting_attachment();
}
