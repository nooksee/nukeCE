    ...
    if ($mode == 'user')
    {
        ...
        else
        {
            ...
            if ($db->sql_numrows($result) == 0)
            {
                ...
                $sql = 'INSERT INTO ' . QUOTA_TABLE . ' ' . attach_sql_build_array('INSERT', $sql_ary);
            }
            else
            {
...
function collect_attachments()
{
    ...
    if (!intval($attach_config['allow_ftp_upload']))
    {
        ...
        {
            message_die(GENERAL_ERROR, 'Is Safe Mode Restriction in effect? Attachment seems to be unable to collect the Attachments within the upload Directory. Try to use FTP Upload to circumvent this error. Another reason could be that the directory ' . $upload_dir . ' does not exist.');
        }
    }
    else
    {
...
function search_attachments($order_by, &$total_rows)
{
    ...
    if ($search_author != '')
    {
        ...
        $search_author = str_replace('*', '%', attach_sql_escape($search_author));
    ...
    if ($search_keyword_fname != '')
    {
        ...
        $where_sql[] = " (a.real_filename LIKE '" . attach_sql_escape($match_word) . "') ";
    }

    if ($search_keyword_comment != '')
    {
        $match_word = str_replace('*', '%', $search_keyword_comment);
        $where_sql[] = " (a.comment LIKE '" . attach_sql_escape($match_word) . "') ";
    }
