...
function physical_filename_already_stored($filename)
{
    ...
    $sql = 'SELECT attach_id 
        ...
        WHERE physical_filename = '" . attach_sql_escape($filename) . "' 
        LIMIT 1";
...
/**
* Sync Topic (NUKE_FORUMS_DIR.'includes/functions_admin.php')
*/
...
/**
* Realpath replacement for Attachment
*/
...
* Escaping SQL
*/
function attach_sql_escape($text)
{
    switch (SQL_LAYER)
    {
        case 'postgresql':
...
*/
function attach_sql_build_array($query, $assoc_ary = false)
{
    ...
    else if ($query == 'UPDATE' || $query == 'SELECT')
    {
        ...
        foreach ($assoc_ary as $key => $var)
        {
            ...
            elseif (is_string($var))
            {
                $values[] = "$key = '" . attach_sql_escape($var) . "'";
            }
            else
            {
