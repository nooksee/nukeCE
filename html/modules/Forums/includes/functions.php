...
if (!defined('IN_PHPBB') && !defined('NUKE_CE'))
{
    die('Hacking attempt');
}
 ...
 ******************************************************/
    include(NUKE_FORUMS_DIR.'includes/lang_extend_mac.'.$phpEx);
/*****[END]********************************************
 [ Mod:    Simply Merge Threads                v1.0.1 ]
 ******************************************************/
    //
...
function phpbb_realpath($path)
{
    ...
    return (!@function_exists('realpath') || !@realpath(NUKE_FORUMS_DIR.'includes/functions.'.$phpEx)) ? $path : @realpath($path);
}
...
function resize_avatar($avatar_url) {
    //moved to includes/functions.php
