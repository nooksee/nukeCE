...
if ($mode == 'mod_edit') {
    ...
    $template->set_filenames(array('body' => 'admin/stat_edit_module.tpl'));
    ...
    for ($i = 0; $i < $num_rows; $i++) {
        if (isset($HTTP_POST_VARS[trim($rows[$i]['config_name'])])) {
            if (trim($HTTP_POST_VARS[trim($rows[$i]['config_name'])]) != trim($rows[$i]['config_value'])) {
                $sql = "UPDATE " . MODULE_ADMIN_TABLE . " SET config_value = '" . trim($HTTP_POST_VARS[trim($rows[$i]['config_name'])]) . "' WHERE config_name = '" . trim($rows[$i]['config_name']) . "' AND module_id = " . $module_id;
...
if ($mode == 'mod_edit') {
    ...
    $sql = "SELECT g.group_id, g.group_name FROM " . GROUPS_TABLE . " g, " . MODULE_GROUP_AUTH_TABLE . " m WHERE m.group_id = g.group_id AND m.module_id = " . intval($mod_info['module_id']) . " ORDER BY group_name";
    ...
    $template->assign_vars(array(
        ...
        'L_MODULE_INFORMATION' => $lang['Module_information'],
 ...
 ******************************************************/
    ...
    for ($i = 0; $i < $num_rows; $i++) {
        ...
        $template->assign_block_vars('module_admin_fields', array('L_TITLE' => $lang_title, 'L_EXPLAIN' => $lang_explain, 'S_OPTION_FIELD' => $option_field));
    }
} else if ($mode == 'select_module') {
    $template->set_filenames(array('body' => 'admin/stat_select_module.tpl'));
    ...
    $template->assign_vars(array('L_SELECT_MODULE_TITLE' => $lang['Module_select_title'], 'L_SELECT_MODULE_EXPLAIN' => $lang['Module_select_explain'], 'L_MODULE_SELECT' => $lang['Module_select_title'], 'L_EDIT' => $lang['Edit'], 'S_ACTION' => append_sid('admin_edit_module.'.$phpEx.'?mode=mod_edit'), 'S_MODULE_SELECT' => $module_select_field));
