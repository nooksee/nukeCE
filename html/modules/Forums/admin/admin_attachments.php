...
@include_once($phpbb_root_path . 'attach/includes/functions_attach.'.$phpEx);
    ...
    if ($submit && ($mode == 'manage' || $mode == 'cats'))
    {
        ...
        if ($config_name == 'max_filesize')
        {
            ...
            $sql = "UPDATE " . ATTACH_CONFIG_TABLE . "
                SET    config_value = '" . attach_sql_escape($new_attach[$config_name]) . "'
                WHERE config_name = '" . attach_sql_escape($config_name) . "'";
        }
        else
        {
            $sql = "UPDATE " . ATTACH_CONFIG_TABLE . "
...
if ($submit && $mode == 'quota')
{
	...
	for ($i = 0; $i < sizeof($quota_change_list); $i++)
    {
        ...
        $sql = 'UPDATE ' . QUOTA_LIMITS_TABLE . "
            SET quota_desc = '" . attach_sql_escape($quota_desc_list[$i]) . "', quota_limit = " . (int) $filesize_list[$i] . "
    ...
    if ($quota_desc != '' && $add)
    {
        ...
        if (!$error)
        {
            ...
            VALUES ('" . attach_sql_escape($quota_desc) . "', " . (int) $filesize . ")";
