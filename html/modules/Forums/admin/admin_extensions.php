    ...
    if ($num_rows > 0)
    {
        for ($i = 0; $i < sizeof($extension_row); $i++)
        {
            if ($extension_row[$i]['comment'] != $extensions['_' . $extension_row[$i]['ext_id']]['comment'] || intval($extension_row[$i]['group_id']) != intval($extensions['_' . $extension_row[$i]['ext_id']]['group_id']))
            {
                ...
                $sql = 'UPDATE ' . EXTENSIONS_TABLE . ' SET ' . attach_sql_build_array('UPDATE', $sql_ary) . '
    ...
    if ($extension != '' && $add)
    {
        ...
        if (!$error)
        {
            ...
            if (!$error)
            {
                ...
                $sql = 'INSERT INTO ' . EXTENSIONS_TABLE . ' ' . attach_sql_build_array('INSERT', $sql_ary);
...
if ($submit && $mode == 'groups')
{
    ...
    for ($i = 0; $i < sizeof($group_change_list); $i++)
    {
        ...
        $sql = 'UPDATE ' . EXTENSION_GROUPS_TABLE . ' SET ' . attach_sql_build_array('UPDATE', $sql_ary) . '
    ...
    if ($extension_group != '' && $add)
    {
        ...
        if (!$error)
        {
            ...
            $sql = 'INSERT INTO ' . EXTENSION_GROUPS_TABLE . ' ' . attach_sql_build_array('INSERT', $sql_ary);
    ...
    if ($extension != '' && $add)
    {
        ...
        if (!$error)
        {
            $sql = 'INSERT INTO ' . FORBIDDEN_EXTENSIONS_TABLE . " (extension)
                VALUES ('" . attach_sql_escape(strtolower($extension)) . "')";
...
if ($add_forum && $e_mode == 'perm' && $group)
{
    ...
    if (!$add_all_forums)
    {
        ...
        $sql = 'UPDATE ' . EXTENSION_GROUPS_TABLE . " SET forum_permissions = '" . attach_sql_escape($auth_bitstream) . "' WHERE group_id = " . (int) $group;
