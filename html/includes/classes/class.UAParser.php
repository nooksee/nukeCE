...
if (!function_exists('json_decode') || !function_exists('json_encode')) {
	require_once(dirname(__DIR__)."/functions_json.php");
}

// load spyc as a YAML loader
require_once(__DIR__."/class.spyc.php");
...
class UA {
	...
	*/
	public static function parse($ua = NULL) {
		...
		if (file_exists(dirname(__DIR__)."/resources/regexes.yaml")) {
			self::$regexes = Spyc::YAMLLoad(dirname(__DIR__)."/resources/regexes.yaml");
		} else {
			print "<h1>Error</h1>
				   <p>Please download the regexes.yaml file before using class.UAParser.php.</p>
				   <p>You can type the following at the command line to download the latest version:</p>
				   <blockquote>
					<code>%: cd /includes/classes/class.UAParser</code><br />
				   	<code>%: php class.UAParser.php -get</code>
				   </blockquote>";
			exit;
		}
	...
	*/
	private static function log($data) {
		...
		$fp = fopen(dirname(__DIR__)."/log/user_agents.log", "a");
		fwrite($fp, $jsonData."\r\n");
		fclose($fp);
	}
		...
		if ($data = @file_get_contents("https://raw.github.com/tobie/ua-parser/master/regexes.yaml")) {
			if (file_exists(dirname(__DIR__)."/resources/regexes.yaml")) {
				if (!self::$nobackup) { 
					...
					if (!copy(dirname(__DIR__)."/resources/regexes.yaml", dirname(__DIR__)."/resources/regexes.".date("Ymdhis").".yaml")) {
						if (!self::$silent) { print("back-up failed...\n"); }
						exit;
					}
				}
			}
			$fp = fopen(dirname(__DIR__)."/resources/regexes.yaml", "w");
			fwrite($fp, $data);
			fclose($fp);
...
if (defined('STDIN') && isset($argv) && isset($argv[1]) && ($argv[1] == '-get')) {
	...
	print("You must use the -get flag to use class.UAParser.php from the command line.\n");
}
