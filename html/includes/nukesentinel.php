...
if($ab_config['proxy_switch'] == 1) {
    ...
    if($ab_config['proxy_switch'] == 1 AND ($proxy1 != "none" OR $proxy2 != "none")) {
        ...
        $display_page = eregi_replace("</body>", "<hr noshade='noshade' />\n<div align='right'>"._AB_SENTINEL." ".$ab_config['version_number']." "._AB_BYNSN."</div>\n</body>", $display_page);
        die($display_page);
    }
    //Mild:
...
if($blocker_row['activate'] > 0) {
    ...
    if (string_bypass('feed=news') && string_bypass('name=gallery2')) {
        ...
        if ( ((stristr($nsnst_const['query_string'], "http://") || stristr($nsnst_const['query_string'], "https://")) && substr($_SERVER['REQUEST_URI'],0,strlen("/index.php?url=")) != '/index.php?url=') OR (stristr($nsnst_const['query_string'], "cmd=") AND !stristr($nsnst_const['query_string'], "&cmd")) OR (stristr($nsnst_const['query_string'], "exec") AND !stristr($nsnst_const['query_string'], "execu")) OR stristr($nsnst_const['query_string'],"concat") AND !stristr($nsnst_const['query_string'], "../")) {
            block_ip($blocker_row);
        }
    }
}
...
if(!is_admin() && string_bypass('feed=news') && string_bypass('name=gallery2')) {
    ...
    if($blocker_row['activate'] > 0) {
        foreach($_GET as $sec_key => $secvalue) {
            if((eregi("<[^>]script*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*object*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*iframe*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*applet*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*meta*\"?[^>]*>", $secvalue)) || (eregi("<[^>]style*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*form*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*img*\"?[^>]*>", $secvalue)) || (eregi("<[^>]*onmouseover*\"?[^>]*>", $secvalue)) || (eregi("<[^>]body*\"?[^>]*>", $secvalue)) || (eregi("\([^>]*\"?[^)]*\)", $secvalue)) || (eregi("\"", $secvalue)) || (eregi("forum_admin", $sec_key)) || (eregi("inside_mod", $sec_key))) {
                block_ip($blocker_row);
            }
        }
        foreach($_POST as $secvalue) {
            if ((eregi("<[^>]*iframe*\"?[^>]*", $secvalue)) || (eregi("<[^>]*object*\"?[^>]*", $secvalue)) || (eregi("<[^>]*applet*\"?[^>]*", $secvalue)) || (eregi("<[^>]*meta*\"?[^>]*", $secvalue)) || (eregi("<[^>]*onmouseover*\"?[^>]*", $secvalue)) || (eregi("<[^>]script*\"?[^>]*", $secvalue)) || (eregi("<[^>]body*\"?[^>]*", $secvalue)) || (eregi("<[^>]style*\"?[^>]*", $secvalue))) {
                block_ip($blocker_row);
            }
        }
    }
...
if($ab_config['track_active'] == 1) {
    ...
    if($pg != "/rss.php" AND $pg != '/modules.php' AND !stristr($pg, "op=gfx")) {
        $result = $db->sql_query("SELECT ip_from FROM `".$prefix."_country_ip` LIMIT 0,1");
        $checkrow = $db->sql_numrows($result);
        $db->sql_freeresult($result);
        if($checkrow > 0) {
           $tresult = $db->sql_query("SELECT `cc2` FROM `".$prefix."_country_ip` WHERE `ip_from`<='".$nsnst_const['remote_long']."' AND `ip_to`>='".$nsnst_const['remote_long']."'");
...
function write_ban($banip, $htip, $blocker_row) {
    ...
    if((!isset($admin) || empty($admin)) || $admin_row['protected'] < 1) {
        if(($blocker_row['activate'] > 3 AND $blocker_row['activate'] < 6) OR $blocker_row['activate'] > 7) {
            ...
            $addby = _AB_ADDBY." "._AB_SENTINEL;
            $querystring = base64_encode($query_url);
            $getstring = base64_encode($get_url);
            $poststring = base64_encode($post_url);
            $checkrow = $db->sql_numrows($db->sql_query("SELECT * FROM `".$prefix."_country_ip`"));
            if($checkrow > 0) {
                list($c2c) = $db->sql_fetchrow($db->sql_query("SELECT `cc2` FROM `".$prefix."_country_ip` WHERE `ip_from`<='".$nsnst_const['remote_long']."' AND `ip_to`>='".$nsnst_const['remote_long']."'"));
            }
            if(!$c2c) { $c2c = "00"; }
            if (!get_magic_quotes_runtime()) {
...
function write_mail($banip, $blocker_row, $abmatch="") {
    ...
    if($blocker_row['activate'] > 0 AND $blocker_row['activate'] < 6) {
        ...
        for($i=0, $maxi=count($admincontact); $i < $maxi; $i++) {
            ...
            @nuke_mail($admincontact[$i], $subject, $message,"From: $admincontact[$i]\r\nX-Mailer: "._AB_SENTINEL);
        }
    }
}
...
function abget_template($template="") {
    ...
    if(!file_exists($filename)) { $filename = NUKE_INCLUDE_DIR."abuse/abuse_default.tpl"; }
...
function PMA_auth_check() {
    ...
    if (empty($PHP_AUTH_USER) && empty($PHP_AUTH_PW) && function_exists('base64_decode')) {
        ...
        } else if (!empty($_ENV) && isset($_ENV['HTTP_AUTHORIZATION']) && substr($_ENV['HTTP_AUTHORIZATION'], 0, 6) == 'Basic ') {
            list($PHP_AUTH_USER, $PHP_AUTH_PW) = explode(':', base64_decode(substr($_ENV['HTTP_AUTHORIZATION'], 6)));
        } else if (@getenv('HTTP_AUTHORIZATION') && substr(getenv('HTTP_AUTHORIZATION'), 0, 6) == 'Basic ') {
